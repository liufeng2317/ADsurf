...
def dltar4_matrix(vlist, tlist, d, a, b, rho, llw):
    ...
    tlist = np.repeat(tlist,vlist.shape[1],1) # [station,clist,t]
    vlist = np.repeat(vlist,tlist.shape[-1],-1) #[station,clist,t]
    omega = tlist
    wvno = 1/vlist*omega
    # Preallocate Dunkin's matrix [station,vc,t,5,5]
    ca = torch.empty((omega.shape[0],omega.shape[1],omega.shape[2],5, 5))

    # Dukin's temp matrix [station,vc,t,5]
    e = torch.zeros((omega.shape[0],omega.shape[1],omega.shape[2],a.shape[-1],5))
    ...
    for m in range(d.shape[-1] - 2, llw, -1):
        ...
        # multi layer 
        ee = torch.zeros(ca.shape[0],ca.shape[1],ca.shape[2],1,5) # [m,n,1,5]
